...
function Edit() {
      ...
      (await imageList.map(async (i: { data_url: string }) => {
        mutationUploadPhotos.mutate(
          { photo_base64: i.data_url.split(",")[1] },
          {
            ...
            onError: () => {
              message.error("Не удалось добавить фото");
            }
          }
        );
      }));
  ...
  const handleDeleteProduct = () => {
    deleteMutation.mutate(
      { product_id, user_id: id },
      {
        ...
        onError: () => {
          message.error("Не удалось удалить товар. Попробуйте ещё раз");
        }
      }
    );
  };
  ...
  return (
    <Container backwardUrl="/admin/products" title="Редактировать товар">
