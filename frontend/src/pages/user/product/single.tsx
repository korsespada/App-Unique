  ...
  const handleAddToCart = () => {
    mutation.mutate(
      ...
      },
      {
        ...
        onError: () => {
          message.error("Ошибка при добавлении в корзину. Попробуйте позже");
        }
      }
    );
  };
    ...
    <Container title={data?.product_Name || ""} backwardUrl={-1}>
      {isFetching || isLoading ? (
        ...
        <div className="flex flex-col gap-3">
          {data?.quantity === 0 && (
            <Alert message="Товара нет в наличии" type="error" showIcon />
          )}
          ...
          <div className=" my-6 text-left">{data?.product_Name}</div>
          <div className="flex w-full flex-row items-center justify-start gap-3">
            ...
            <div
              className={`flex flex-row items-center gap-2 self-end text-left ${
                finalPrice && " text-gray-500 line-through"
              }`}>
              <span>₽</span> <span>{addCommas(data?.price) || 0}</span>
            </div>
            {finalPrice && (
              <div className="flex flex-row items-center gap-2 self-end text-left">
                <span>₽</span>{" "}
