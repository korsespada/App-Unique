...
function ProductItem({
  ...
  return (
    ...
    <div
      className={`${quantity === 0 && "opacity-50 grayscale"} ${
      ...
      <div className="flex w-2/3 flex-col items-center justify-between p-2">
        <p className="mb-1 mr-auto h-5 w-full select-none text-left ">
          {title}
        </p>
        <div className="flex w-full  flex-col gap-2">
          {/* <div className="flex items-center justify-between">
            ...
            <div className="select-none text-sm">Категория</div>
          </div> */}
          <div
            className={`flex flex-row gap-3 self-end text-left ${
              finalPrice && " text-sm text-gray-500 line-through"
            }`}>
            <span>₽</span> <span>{addCommas(price)}</span>
          </div>
          {finalPrice && (
            <div className="flex flex-row gap-3 self-end text-left">
              <span>₽</span> <span>{addCommas(discountedPrice)}</span>
            </div>
          )}
          {pageType === "admin" && (
      ...
      <div
        className=" relative mr-auto h-full  w-1/3 bg-[var(--tg-theme-secondary-bg-color)] bg-cover"
        style={{
