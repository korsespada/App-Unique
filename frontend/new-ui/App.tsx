...
const App: React.FC = () => {
    ...
    <div className="pb-32 pt-28 animate-in fade-in duration-700">
      ...
      <div className="px-6 mb-8">
        <div className="relative">
          ...
          />
          <Search size={18} className="absolute right-5 top-1/2 -translate-y-1/2 text-neutral-300" />
        </div>
      </div>
      ...
      <div className="px-6 mb-8 overflow-x-auto whitespace-nowrap scrollbar-hide flex gap-8 items-center">
        {CATEGORIES.map(cat => (
          <button
            ...
            className={`text-[10px] uppercase tracking-[0.25em] transition-all font-black ${activeCategory === cat ? 'text-black scale-110' : 'text-neutral-300'}`}
          >
            {cat}
          </button>
        ))}
      ...
      <div className="px-6 mb-10 flex items-center justify-between gap-4">
        <div className="relative flex-1 group">
          ...
          <ChevronDown size={14} className="absolute right-5 top-1/2 -translate-y-1/2 text-neutral-400 pointer-events-none group-hover:text-black transition-colors" />
        </div>
      ...
      <div className="px-6 grid grid-cols-2 gap-x-5 gap-y-10">
        {filteredAndSortedProducts.length > 0 ? (
          filteredAndSortedProducts.map(product => (
            <div key={product.id} className="cursor-pointer active:scale-95 transition-transform duration-300" onClick={() => navigateToProduct(product)}>
              ...
              <div className="px-2">
                <p className="text-[9px] uppercase tracking-[0.2em] text-neutral-400 font-black mb-1">{product.brand}</p>
                <h3 className="text-sm font-bold tracking-tight text-neutral-900 line-clamp-1">{product.name}</h3>
              </div>
            </div>
          ))
        ) : (
          <div className="col-span-2 py-40 text-center">
            <p className="text-neutral-300 text-sm font-medium tracking-wide">Ничего не найдено</p>
          </div>
        )}
      </div>
      ...
      {isSortOpen && (
        <div className="fixed inset-0 z-[100] flex items-end">
          <div className="absolute inset-0 bg-black/60 backdrop-blur-md" onClick={() => setIsSortOpen(false)} />
          <div className="relative w-full bg-white rounded-t-[3rem] p-10 animate-in slide-in-from-bottom duration-500 shadow-2xl">
            ...
            <div className="flex justify-between items-center mb-12">
              ...
              <button onClick={() => setIsSortOpen(false)} className="p-3 bg-neutral-50 rounded-full text-neutral-400 hover:text-black transition-all">
                <X size={20} />
              </button>
            </div>
            <div className="space-y-4">
              {[
                ...
                <button 
                  ...
                  className={`w-full py-6 px-8 rounded-3xl text-left flex justify-between items-center transition-all ${sortOption === opt.id ? 'bg-black text-white shadow-xl translate-x-2' : 'bg-neutral-50 text-neutral-500 hover:bg-neutral-100'}`}
                >
                  <span className="text-xs uppercase tracking-[0.2em] font-black">{opt.label}</span>
                  {sortOption === opt.id && <div className="w-2.5 h-2.5 bg-white rounded-full animate-pulse" />}
                </button>
              ))}
            </div>
          </div>
    ...
    return (
      <div className="pb-48 animate-in slide-in-from-right duration-500 bg-white">
        ...
        <div className="fixed top-0 max-w-md w-full px-6 py-6 flex items-center justify-between z-[100] blur-nav">
          <button onClick={() => setCurrentView('home')} className="p-3 bg-white/50 border border-neutral-100 rounded-2xl premium-shadow hover:scale-110 transition-transform">
            <ArrowLeft size={20} />
          </button>
          <button className="p-3 bg-white/50 border border-neutral-100 rounded-2xl premium-shadow">
            <ShoppingBag size={20} />
          </button>
        </div>
            ...
            <div className="absolute bottom-12 left-0 right-0 px-6 flex justify-between items-center pointer-events-none">
              ...
              <div className="flex gap-2.5">
                {selectedProduct.images.map((_, i) => (
                  <div key={i} className={`h-1.5 transition-all duration-500 rounded-full ${i === currentImageIndex ? 'w-8 bg-black' : 'w-2 bg-black/10'}`} />
                ))}
              </div>
              <button 
        ...
        <div className="px-8 -mt-8 relative z-10 bg-white rounded-t-[3rem] pt-12">
          <div className="flex justify-between items-start mb-10">
            <div className="max-w-[70%]">
              <p className="text-[10px] uppercase tracking-[0.4em] text-neutral-400 font-black mb-3">{selectedProduct.brand}</p>
              <h2 className="text-4xl font-black leading-none tracking-tighter">{selectedProduct.name}</h2>
            </div>
            <div className="text-right">
              <p className="text-2xl font-black text-black">{selectedProduct.price.toLocaleString()} ₽</p>
              <div className="h-1 w-8 bg-black ml-auto mt-2" />
            </div>
          </div>
          ...
          <div className="space-y-8 mb-12">
            <h4 className="text-[11px] uppercase tracking-[0.35em] font-black text-neutral-300">Особенности</h4>
            <div className="grid grid-cols-1 gap-5">
              {selectedProduct.details.map((detail, i) => (
                <div key={i} className="flex items-center gap-5 p-6 bg-neutral-50 rounded-[2rem] border border-neutral-100 transition-all hover:translate-x-2">
                  <div className="w-10 h-10 bg-white rounded-2xl flex items-center justify-center premium-shadow font-black text-xs text-neutral-300">0{i+1}</div>
  ...
  const CartView = () => (
    <div className="pt-28 pb-32 px-8 min-h-screen animate-in fade-in duration-500">
      <div className="flex justify-between items-end mb-12">
        <h2 className="text-5xl font-black tracking-tighter">Сумка</h2>
      ...
      {cart.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-40 text-center">
          ...
          <p className="text-neutral-400 mb-12 font-bold tracking-tight">Ваша корзина пуста</p>
          <button 
        ...
        <>
              ...
              <div key={item.id} className="flex gap-8 items-center group">
                ...
                <div className="flex-1 flex flex-col gap-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="text-[9px] uppercase tracking-[0.3em] text-neutral-400 font-black mb-1">{item.brand}</p>
                      <h4 className="text-[15px] font-black leading-tight mb-1">{item.name}</h4>
                      <p className="text-sm font-bold text-neutral-800">{item.price.toLocaleString()} ₽</p>
                    </div>
                    <button onClick={() => updateQuantity(item.id, -item.quantity)} className="text-neutral-200 hover:text-red-500 transition-colors p-2">
                      <Trash2 size={20} />
                    </button>
                  </div>
                  <div className="flex items-center gap-4 bg-neutral-50 self-start px-2 py-1.5 rounded-2xl border border-neutral-100">
                    <button onClick={() => updateQuantity(item.id, -1)} className="p-2 hover:bg-white rounded-xl transition-all"><Minus size={14} /></button>
                    <span className="text-xs font-black min-w-[20px] text-center">{item.quantity}</span>
                    <button onClick={() => updateQuantity(item.id, 1)} className="p-2 hover:bg-white rounded-xl transition-all"><Plus size={14} /></button>
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="p-10 bg-black text-white rounded-[3.5rem] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.5)] mb-10">
            <div className="flex justify-between items-center mb-2">
              <span className="text-neutral-400 text-[10px] uppercase tracking-[0.4em] font-black">Сумма заказа</span>
              <span className="text-3xl font-black">{cartTotal.toLocaleString()} ₽</span>
            </div>
            <p className="text-[10px] text-neutral-600 mb-12 uppercase tracking-widest font-black">включая доставку из Милана</p>
            <button className="w-full py-6 bg-white text-black text-[11px] uppercase tracking-[0.4em] font-black rounded-2xl active:scale-95 transition-all shadow-xl hover:bg-neutral-100">
              Оформить
            </button>
          </div>
        </>
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen relative overflow-x-hidden">
      {/* Dynamic Navbar */}
      <nav className={`fixed top-0 max-w-md w-full z-[120] flex items-center justify-between px-8 py-7 transition-all duration-500 ${scrolled || currentView !== 'home' ? 'blur-nav bg-white/80 py-4 border-b border-neutral-100' : 'bg-transparent'}`}>
        <div 
          onClick={() => { setCurrentView('home'); window.scrollTo({top:0, behavior:'smooth'}); }} 
          className="text-xl tracking-[0.3em] cursor-pointer hover:opacity-50 transition-all font-black text-black"
        >
          YEEZY<span className="text-neutral-300">UNIQUE</span>
        </div>
        {currentView === 'home' && (
          <div className="relative cursor-pointer" onClick={() => setCurrentView('cart')}>
            <ShoppingBag size={22} />
            {cartCount > 0 && <span className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-black text-white text-[8px] font-black rounded-full flex items-center justify-center border border-white">{cartCount}</span>}
          </div>
        )}
      </nav>
      ...
      <div className="fixed bottom-0 max-w-md w-full px-12 pb-6 pt-2 z-[110] flex justify-center">
        <div className="bg-black/95 blur-nav px-12 py-3 rounded-[2rem] flex items-center gap-16 shadow-2xl border border-white/10">
          ...
          >
            ...
            {cartCount > 0 && <span className="absolute -top-1 -right-1 bg-white text-black text-[6px] w-3 h-3 rounded-full flex items-center justify-center font-black">{cartCount}</span>}
          </button>
        </div>
      </div>
    </div>
